...
app.use(cors({
  origin: (origin, callback) => {
    ...
    // In production, we typically want to allow the same domain
    // or specific origins from environment variables.
    const allowedOrigins = [process.env.CLIENT_ORIGIN].filter(Boolean);
    if (allowedOrigins.length === 0 || allowedOrigins.includes(origin)) {
      return callback(null, true);
    }
    
    // Default to allow for Vercel preview/production domains
    if (origin.endsWith(".vercel.app")) {
      return callback(null, true);
    }

    callback(null, true); // Fallback to allow all during debug, change to restricted later
  },
  credentials: true 
}));
...
app.post("/api/auth/login", async (req, res) => {
  try {
    ...
    if (email !== user.email) {
      console.log("Email mismatch. Provided:", email, "Expected:", user.email);
...
app.post("/api/contact", async (req, res) => {
  ...
  try {
    ...
    // In Resend Testing Mode, the 'to' address MUST be the account email.
    const notificationEmail = "fosterboadi152@gmail.com";
    
    console.log("Sending notification to verified account email:", notificationEmail);
    ...
    const { data, error } = await resend.emails.send({
      ...
      html: `
        <div style="font-family: sans-serif; line-height: 1.5; color: #333;">
          ...
          <p style="font-size: 0.8rem; color: #666;">Sent from your Developer Portfolio on Vercel.</p>
        </div>
      `,
    });
